...
int CExplorerBrowserView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
			hr = m_spExplorerBrowser->SetPropertyBag(L"MyPropertyBag");
...
void CExplorerBrowserView::OnViewFrames()
{
    ...
    //m_spExplorerBrowser->SetOptions(m_bShowFrames ? (int)EBO_SHOWFRAMES : 0);
	m_spExplorerBrowser->SetOptions(EBO_SHOWFRAMES);
}
...
STDMETHODIMP CExplorerBrowserView::OnViewCreated(IShellView *psv)
{
	HRESULT							hr;
	CComPtr<IFolderViewOptions>		pfolderViewOptions;
	FOLDERVIEWOPTIONS				fvoFlags;
	FOLDERVIEWOPTIONS				fvoFlagsTest;

	hr = m_spExplorerBrowser->QueryInterface(IID_IFolderViewOptions, (void**)& pfolderViewOptions);

	if (SUCCEEDED(hr))
	{
		// Cx sets the folder view options to custom ordering
		pfolderViewOptions->GetFolderViewOptions(&fvoFlags);

		fvoFlagsTest = fvoFlags & FVO_CUSTOMORDERING;

		if (fvoFlagsTest != FVO_CUSTOMORDERING)
		{
			hr = pfolderViewOptions->SetFolderViewOptions(FVO_CUSTOMORDERING, FVO_CUSTOMORDERING);
		}
	}
